...
function intersectPlane(ray, plane) {
	...
	const normal = new Vector3(plane.normal.x, plane.normal.y, plane.normal.z).normalize();
...
function calculateLighting(point, normal, material, viewDir, lights, objects, ambientLight) {
	let color = new Vector3(0, 0, 0);
	const albedo = new Vector3(material.albedo.x, material.albedo.y, material.albedo.z);
	...
	for (const light of lights) {
		const lightPos = new Vector3(light.position.x, light.position.y, light.position.z);
	...
	if (material.emission) {
		const emission = new Vector3(material.emission.x, material.emission.y, material.emission.z);
...
function trace(ray, objects, lights, backgroundColor, ambientLight, maxBounces, depth = 0) {
	if (depth >= maxBounces) {
...
function getRay(x, y, width, height, camera) {
	...
	const horizontal = new Vector3(camera.horizontal.x, camera.horizontal.y, camera.horizontal.z);
	const vertical = new Vector3(camera.vertical.x, camera.vertical.y, camera.vertical.z);
...
self.onmessage = function (e) {
	...
	const bgColor = new Vector3(backgroundColor.x, backgroundColor.y, backgroundColor.z);
	...
	for (let y = startY; y < endY; y++) {
		for (let x = 0; x < width; x++) {
			...
			for (let sample = 0; sample < antiAliasingSamples; sample++) {
				...
				color = color.add(trace(ray, objects, lights, bgColor, ambientLight, maxBounces));
				rayCount++;
			}
