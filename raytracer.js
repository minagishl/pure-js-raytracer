...
class RayTracer {
	...
	initWorkers() {
		...
		try {
			...
			console.warn('Web Workers not available, falling back to single-threaded rendering:', error);
	...
	setupScene() {
		...
		// Add lights
		this.lights.push(new Light(new Vector3(4, 4, 4), new Vector3(1, 1, 1), 1.0));

		this.lights.push(new Light(new Vector3(-4, 4, 4), new Vector3(0.8, 0.8, 1.0), 0.7));
	}
	...
	trace(ray, depth = 0) {
		...
		if (material.metallic > 0 && depth < this.maxBounces - 1) {
			...
			const reflectionRay = new Ray(point.add(normal.multiply(0.001)), reflectionDir);
		...
		if (material.transparency > 0 && depth < this.maxBounces - 1) {
			const eta =
				ray.direction.dot(normal) < 0 ? 1.0 / material.refractiveIndex : material.refractiveIndex;
			const refractionDir = ray.direction.refract(normal, eta);

			if (refractionDir) {
				const refractionRay = new Ray(point.subtract(normal.multiply(0.001)), refractionDir);
			...
			for (let y = 0; y < this.height; y++) {
				for (let x = 0; x < this.width; x++) {
					...
					for (let sample = 0; sample < this.antiAliasingSamples; sample++) {
						...
						const v = 1.0 - (y + (sample > 0 ? Math.random() : 0.5)) / this.height;
	...
	renderWithWorkers(startTime) {
		...
		const renderTimeout = setTimeout(() => {
			if (completedWorkers < this.numWorkers) {
				console.warn('Worker rendering timed out, falling back to single-threaded');
				this.useWorkers = false;
				this.isRendering = false;
				this.render();
			}
	...
	serializeObject(obj) {
		if (obj.type === 'sphere') {
			return {
				...
				radius: obj.radius,
				material: {
					albedo: { x: obj.material.albedo.x, y: obj.material.albedo.y, z: obj.material.albedo.z },
					...
					emission: obj.material.emission
						? { x: obj.material.emission.x, y: obj.material.emission.y, z: obj.material.emission.z }
						: null,
				},
			};
	...
	serializeCamera() {
		return {
			...
			vertical: { x: this.camera.vertical.x, y: this.camera.vertical.y, z: this.camera.vertical.z },
			lower_left_corner: {
	...
	startPerformanceMonitoring() {
		const updateStats = () => {
			...
			document.getElementById('render-time').textContent = Math.round(this.renderTime) + 'ms';
			document.getElementById('ray-count').textContent = this.rayCount.toLocaleString();

			// Check if rendering has stalled
			if (this.isRendering && currentTime - this.lastFrameTime > 5000) {
				console.warn('Rendering appears to have stalled, attempting recovery...');
				this.isRendering = false;
				this.renderErrors++;
...
class UIController {
	...
	setupControls() {
		...
		document.getElementById('resolution').addEventListener('change', (e) => {
			...
			this.raytracer.resize(Math.floor(rect.width * scale), Math.floor(rect.height * scale));
	...
	updateObjectsList() {
		...
		this.raytracer.objects.forEach((object, index) => {
			...
			if (object.type === 'sphere') {
				panel.innerHTML = `
                    <div class="text-sm font-medium mb-2">Sphere ${index + 1}</div>
                    <div class="control-group">
                        <label>Radius:</label>
                        <input type="range" min="0.1" max="2" step="0.1" value="${object.radius}" 
	...
	updateSphereRadius(index, value) {
		if (this.raytracer.objects[index] && this.raytracer.objects[index].type === 'sphere') {
...
window.addEventListener('DOMContentLoaded', () => {
	...
	window.addEventListener('resize', () => {
		...
		raytracer.resize(Math.floor(newRect.width * scale), Math.floor(newRect.height * scale));
